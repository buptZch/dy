...
type GinJWTMiddleware struct {
	...
	Unauthorized func(c *gin.Context, code int, message string, userId int64, token string)

	RegisterResponse func(c *gin.Context, err error, userId int64, token string)
...
func (mw *GinJWTMiddleware) middlewareImpl(c *gin.Context) {
	...
	if err != nil {
		mw.unauthorized(c, http.StatusUnauthorized+1000, mw.HTTPStatusMessageFunc(err, c))
		return
	}
	...
	if int64(claims["exp"].(float64)) < mw.TimeFunc().Unix() {
		mw.unauthorized(c, http.StatusUnauthorized+2000, mw.HTTPStatusMessageFunc(ErrExpiredToken, c))
		return
	}
...
func (mw *GinJWTMiddleware) RegisterHandler(c *gin.Context) {
	...
	err = rpc.CreateUser(c, &userbase.CreateUserRequest{
...
func (mw *GinJWTMiddleware) unauthorized(c *gin.Context, code int, message string) {
	...
	mw.Unauthorized(c, code, message, 0, "")
}
