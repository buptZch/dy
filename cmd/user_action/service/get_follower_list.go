...
func (s *FollowerListService) FollowerList(req *useraction.RelationFollowerListRequest) ([]*useraction.User, error) {
	modleFollows, err := db.GetFollowerList(s.ctx, req.UserId)
	if err != nil {
		return nil, err
	}

	uList := make([]*useraction.User, 0)
	UIds := pack.Followers(modleFollows)
