...
func NewGetPublishVideoService(ctx context.Context) *GetPublishVideoService {
	return &GetPublishVideoService{ctx: ctx}
}

//服务里把response所有需要的返回值送入handler中装配
func (s *GetPublishVideoService) GetPublishVideo(req *useraction.PublishListRequest) ([]*useraction.Video, error) {

	modelLikes, err := db.GetPublishVideoId(s.ctx, req.UserId)
	if err != nil {
		return nil, err
	}
	vList := make([]*useraction.Video, 0)
	vIds := pack.Videos(modelLikes)
